...
class LegalDocumentIntelligence:
    ...
    def __init__(self, mode: Optional[str] = None) -> None:
        ...
        self._last_ocr_result: Dict[str, Any] = {}
    ...
    def _load_image_text(self, file_path: str) -> str:
        ...
        self._last_ocr_result = result if isinstance(result, dict) else {}
        return self._normalize_text(str(result.get("text", "")))

    def _signature_status_from_text(self, text: str) -> List[Dict[str, Any]]:
        signatures: List[Dict[str, Any]] = []
        seen_locations = set()
        lines = [line.strip() for line in self._normalize_text(text).splitlines() if line.strip()]
        keywords = ("signature", "signed", "signatory", "authorized signatory", "by:", "by ")
        ...
        ocr_signatures = self._last_ocr_result.get("signatures") if isinstance(self._last_ocr_result, dict) else []
        if isinstance(ocr_signatures, list):
            for item in ocr_signatures:
    ...
    def analyze_text(self, text: str, source_name: Optional[str] = None, question: Optional[str] = None) -> Dict[str, Any]:
        ...
        signatures = self._signature_status_from_text(normalized)
        signature_annotated_image_url = None
        ocr_annotated_image_path = None
        if isinstance(self._last_ocr_result, dict):
            ocr_annotated_image_path = self._last_ocr_result.get("annotated_image_path")
            signature_annotated_image_path = self._last_ocr_result.get("signature_annotated_image_path")
            if signature_annotated_image_path:
                signature_annotated_image_url = f"/generated/legal/{Path(signature_annotated_image_path).name}"
            elif ocr_annotated_image_path:
                signature_annotated_image_url = f"/generated/legal/{Path(ocr_annotated_image_path).name}"
        ...
        return {
            ...
            "signature_annotated_image_url": signature_annotated_image_url,
