...
def compute_grad_cam(
    ...
    grad_cam = GradCAM(model, model.img_backbone[0][-1])
    ...
    top_regions_save_path = _save_top_regions_visualization(
        img_np,
        cam_resized,
        save_path,
    )
    ...
    return {
        ...
        "top_regions_save_path": top_regions_save_path,
...
def _save_top_regions_visualization(img_np, cam_resized, base_save_path, top_k=5):
    """Save a visual overlay that marks the top CAM regions on the image."""
    cam = np.asarray(cam_resized)
    flat_cam = cam.flatten()
    top_indices = np.argsort(flat_cam)[-top_k:][::-1]
    ...
    for rank, idx in enumerate(top_indices, start=1):
        ...
        ax.scatter([col], [row], s=180, facecolors="none", edgecolors="white", linewidths=2.0)
        ax.text(
            col + 4,
            row + 4,
            str(rank),
            ...
            fontsize=10,
            weight="bold",
            bbox=dict(facecolor="black", alpha=0.65, edgecolor="none", pad=1.5),
        )

    ax.set_title("Top Grad-CAM Regions")
    ax.axis("off")

    root, ext = os.path.splitext(base_save_path)
    top_regions_path = f"{root}_top_regions{ext}"
    plt.tight_layout()
    plt.savefig(top_regions_path, dpi=150, bbox_inches="tight")
    plt.close()

    return top_regions_path
...
def generate_xai_report(
    ...
    report = {
        ...
        "grad_cam": {
            ...
            "top_regions_save": grad_cam_res["top_regions_save_path"],
